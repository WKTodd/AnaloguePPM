...
;    Date:     17/09/1991 to 28/12/2020                               *
;    File Version:      2.21					      *
;  improvements to DCoffset adjustment	faster adc interrupt          *
;  Phase meter routines added					      *
;                                                                     *
;    Author:             W.K.Todd                                     *
...
PRSgn		EQU		0x2D		
PLtmp		EQU		0x2E
PRtmp		EQU		0x2F	;temp store for peak
...
DCO		EQU		0x75		;DC offset counter for DC filter		
DCRldL		EQU		0x76		;DC offset adjust ing pulse  value
DCRldH		EQU		0x77		
DCTmr		EQU		0x78		;timer for above
PhaseL		EQU		0x79		;phase meter counter
PhaseN		EQU		0x7A
Phase		EQU		0x7B		;phase count +/-50		
...
AdInt:
	    ...
	    movlw	B'00000101'	;preselect Right (to swap ch)   
	    btfss	Control,ADCch	;0=right 1=left
	    movlw	B'00000001'	;preselect Left 
	    movwf	ADCON0		;select other channel (start Tacq)

	    lslf	ADRESL,w		;get bit 1 into carry
	    rlf		ADRESH,w		;get sign bit (9) into carry and 8-1 into w
	    
;	    banksel	CM1CON0
;	    bcf		CM1CON0,C1POL
;	    btfsc	status,c	;test sign bit 0=+ve
;	    bsf		CM1CON0,C1POL	;set comparator for phase meter	    
;	    banksel	ADCON0

	    btfsc	status,c	;test sign bit 1=+ve  
	    bra		positv
	    xorlw	0xff		;rectify
...
;do right channel and DC offset
saveRch	    bsf		Control,ADCch	;toggle input to left ch
	    rrf		PRSgn,f		;save sign as bit 7
	    movwf	PRtmp		;save for later 
...
;do DC offset counter on each right cycle
xitPR	    
	    movf	PLsgn,w	    ;test signs
	    xorwf	PRsgn,w
	    btfsc	Wreg,7	    ;xit if different
	    bra		intT0
	    ...
	    decfsz	DCO,f
	    bra		intT0
	    incf	DCO,f		;dec down to 1
	    bra		intT0
	    
doneg	    incfsz	DCO,f
	    bra		intT0
	    comf	DCO,f		;inc dc offset counter to FF	    
	    bra		intT0
	    
;do left channel and phase	    
	      
saveLch	    bcf		Control,ADCch	;toggle to right ch for next loop
	    rrf		PLsgn,f		;save sign
	    movwf	PLtmp
...
xitPL	    movf	PLtmp,w		;test level for phase routine
	    iorwf	PRtmp,w
	    andlw	0xFC		;test 6 MS bits
	    btfsc	status,z
	    bra		tlfpm		;too low for phase meter
	    
;do phase counter on each left cycle
;	    rlf		PLSgn,w		;get Lch sign into Carry
;	    banksel	CM1CON0		;bank 2
;	    bcf		CM1CON0,C1POL
;	    btfsc	status,c	;test sign bit 0=+ve
;	    bsf		CM1CON0,C1POL	;set comparator for phase meter	 
;	        
;	    btfsc	CM1CON0,C1OUT	;set if L+R sign different
...
outphz	    ;out of phase so dec to -100 (-50)
	    movlw	.100
	    addwf	Phase,w
	    subwf	PhaseL,f
	    clrw
...
tlfpm	    ;inc/dec phase to zero
	    btfss	Phase,7
	    bra		cntm	    ;test negative
	    ;count up to zero
...
;	    call	setTimer	
	    ...
	    movlw	B'00001011'	;RA0 & RA1 & RA2 & RA3 inputs
	    ...
	    movwf	DACCON0
	    movlw	16
	    movwf	DACCON1	    ;set output to 1/2 Vref
	    
	    ;set up comparator
	    banksel	CM1CON0
	    movlw	B'10000100'
	    movwf	CM1CON0
	    movlw	B'00010000' ;+ve in = DACout (1/2 Vref)
	    movwf	CM1CON1
...
DoMode:	    movf	Mode,w
	    ...
	    goto	SetDecay	;mode 2 - set decay rate
	    goto	SetPkCl		;mode 3 - set peak hold times
	    goto	SetDCO		;mode 4 - DC adjust
	    goto	PhaseDot	;mode 1 - Phase meter
	    nop				;mode 5 -  
	    nop				;mode 6 - 
	    goto	PrfSave		;mode 7 - save prefs
RSMode	    clrf	Mode
	    return
...
DCOffset:   
	    ...
	    movlw	.127 ;.117	;test lower threshold
	    subwf	DCO,w
	    btfsc	status,c    
...
;use dac to make offset  - does not work because DAC is not buffered!	    
;	    banksel	PORTA
;	    bcf		disport,latch
;	    banksel	TRISA
;	    bcf		DACCON0,DACOE	;dac output off
;
;	    bcf		TRISA,latch	;switch port to digital out 	
;	    banksel	PORTA
;	    bsf		disport,latch	;latch data
;	    nop
;	    bcf		disport,latch
;	    banksel	TRISA
;	    bsf		TRISA,latch	;switch port to digital in 
;	    bsf		DACCON0,DACOE	;dac output on
;	    
;	    movlb	0
...
FPRTab	    RETLW   0x02	;PREF table store in HEF
	    RETLW   0x02
	    RETLW   .75		;0x4B
	    RETLW   0xB7	;DCPldL offset value
	    RETLW   0x4C	;DCPldH
