...
;start
	    ...
	    movlw	B'10000100' ;TX/RX and PWM2
	    movwf	APFCON
...
;	    ;initialise PWM2 for DC offset adjustment
;	    banksel	PWM2CON
;	    movlw	B'01000001'	;/16 prescale, HFintosc	    
;	    movwf	PWM2CLKCON	;pwm clock control
;	    movlw	B'01000000'	;enable module standard mode
;	    movwf	PWM2CON		;used for Step out pin RA4
;	    
;	    clrf	PWM2OFCON	;offset control
;	    clrf	PWM2LDCON	;PWM load control
;	    clrf	PWM2PHL
;	    clrf	PWM2PHH
;	    clrf	PWM2OFL
;	    clrf	PWM2OFH
;	    clrf   	PWM2TMRH	;timer register Hi
;	    clrf	PWM2TMRL
;	    movlw	HIGH  20000
;	    movwf	PWM2PRH		;period count Hi
;	    movlw	LOW  20000
;	    movwf	PWM2PRL		;period count Lo
;	    movlw	HIGH  10000
;	    movwf	PWM2DCH		;Duty cycle Hi 
;	    movlw	LOW  10000
;	    movwf	PWM2DCL		;Duty cyle Lo 
;	    bsf		PWM2LDCON,LDA 
;	    bsf		PWM2CON,EN
;	    bsf		PWM2CON,OE
;	    
...
DoMode:	    movf	Mode,w
	    ...
	    goto	CentreDot	;mode 3 - DC adjust
...
DCOffset:    ;adjust output of PWM2 to null DC offset	    
	    movf	Phase,w
	    sublw	.117 ;.55		;test lower threshold
	    btfss	status,c    
	    bra		DCOuppr
...
DCOuppr	    movf	Phase,w
	    sublw	.137;.200		;test upper threshold
	    btfsc	status,c    
	    return
;	    ;raise duty cycle to 0xFFFF max
;	    banksel	PWM2DC
;	    incf	PWM2DCL,f
;	    btfsc	status,c
;	    incfsz	PWM2DCH,f
...
waitlst     btfss	TXSTA,TRMT	    
	    ...
	    ;disable UART o/p and enable PWM
	    ;Banksel	APFCON
	    ;bcf		APFCON,TXCKSEL 
	    Banksel	RCSTA	    ;BANK 3
	    bcf		RCSTA,SPEN
